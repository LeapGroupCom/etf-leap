...
const extractOperationName = (document: TypedDocumentNode<unknown, unknown>): string | undefined => {
...
export async function fetchGraphQL<Operation extends TypedDocumentNode<any, any>>(
	operation: Operation,
	...
	try {
		const auth = Buffer.from(`${serverEnv.WP_USER}:${serverEnv.WP_APP_PASSWORD}`).toString('base64')
		...
		if (errors) {
			...
			throw new FetchError('GraphQL response contains errors', response, errors)
		}
