...
function ChartTooltipContent({
	...
	return (
		...
		>
			...
			{withTotal && payload[0].payload.endBalance && (
				<div className="mt-1.5 flex basis-full items-center border-t pt-1.5 text-xs font-medium text-foreground">
					Total
					<div className="ml-auto flex items-baseline gap-0.5 font-mono font-medium tabular-nums text-foreground">
						{payload[0].payload.endBalance.toLocaleString()}
					</div>
				</div>
			)}
		</div>
...
function CustomizedAxisTick({ x, y, stroke, payload }: any) {
	return (
		<g transform={`translate(${x},${y})`}>
			<text x={0} y={0} dy={16} textAnchor="end" fill="#666" transform="rotate(-90)" stroke={stroke}>
				{payload.value}
			</text>
		</g>
	)
